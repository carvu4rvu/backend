...
const server = app.listen(PORT, '0.0.0.0', async () => {
  ...
  const connectWithRetry = async (name, connectFn, attempts = 3, delay = 2000) => {
  ...
  try {
    ...
    // Supabase Storage often has cold starts or intermittent network issues; use more attempts and longer delay
    ...
    if (process.env.NODE_ENV === 'production') {
      await connectWithRetry('supabase storage', storageCheck, 5, 5000);
    } else {
      try {
        await connectWithRetry('supabase storage', storageCheck, 5, 5000);
      } catch (err) {
