...
/** Semester rows must have at least one course line with code, title, or numeric credits (matches save validation). */
function semesterHasMeaningfulCourses(courses) {
...
exports.getAcademicSemesters = async (req, res) => {
  ...
  try {
    ...
    const filtered = withCourses.filter((s) => {
      const semNum = Number(s.semester);
      if (Number.isNaN(semNum) || s.academic_year == null || s.academic_year === '') return false;
      return semesterHasMeaningfulCourses(s.courses);
    });

    return res.json({ semesters: filtered });
  } catch (error) {
...
/**
 * DELETE /student/profile/:usn/academic-semesters/:id
 * Deletes a single semester record and its courses.
 */
exports.deleteAcademicSemester = async (req, res) => {
  const client = await pool.connect();
  try {
    const { usn, id } = req.params;
    if (!usn || !id) {
      return res.status(400).json({ error: 'USN and semester record ID are required.' });
    }

    await client.query('BEGIN');

    // 1. Get semester number before deleting for lock check and recompute
    const semResult = await client.query(
      `SELECT semester FROM public.student_semester_records WHERE id = $1 AND usn = $2`,
      [id, usn.toUpperCase()]
    );

    if (semResult.rowCount === 0) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: 'Semester record not found.' });
    }

    const semNum = semResult.rows[0].semester;

    // 2. Check if semester is locked (unless it's an admin)
    try {
      ...
      // If user is not admin/vc and semester is locked, prevent delete
      const isAdmin = req.user?.role === 'admin' || req.user?.role === 'vc';
      if (!isAdmin && lockCheck?.rows?.[0]?.locked === true) {
        await client.query('ROLLBACK');
        return sendLocked(res, `Semester ${semNum} is locked. You cannot delete it.`);
      }
    } catch (e) {
      // Ignore lock check if control table doesn't exist
    }

    // 3. Delete courses first (FK constraint)
    await client.query(
      `DELETE FROM public.student_course_wise_academics WHERE semester_record_id = $1`,
      [id]
    );

    // 4. Delete semester record
    await client.query(
      `DELETE FROM public.student_semester_records WHERE id = $1 AND usn = $2`,
      [id, usn.toUpperCase()]
    );
    ...
    await client.query('ROLLBACK').catch(() => {});
    // eslint-disable-next-line no-console
    console.error('deleteAcademicSemester error:', error);
