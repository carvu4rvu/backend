...
/**
 * Delete a school (admin). Allowed only if no students are associated, 
 * UNLESS cascade=true is passed (deletes students too).
 */
exports.deleteSchool = async (req, res) => {
    ...
    try {
        ...
        const studentCheck = await client.query(
            'SELECT count(*) FROM student_basic_details WHERE school_id = $1',
            [id]
        );
        const studentCount = parseInt(studentCheck.rows[0].count, 10);

        if (studentCount > 0 && !cascade) {
            await client.query('ROLLBACK');
            return sendConflict(res, 'Cannot delete school: it has students associated. Use cascade=true to delete them all.');
        }

        if (studentCount > 0 && cascade) {
            await deleteStudentsByCriteria(client, 'school_id = $1', [id]);
        }

        // Delete associated minors, programs etc if they exist (they might have their own students)
        // For simplicity, we delete them all.
        await client.query('DELETE FROM minors WHERE school_id = $1', [id]);
        
        // Before deleting programs, we need to clear their students if cascading
        if (cascade) {
            const programs = await client.query('SELECT id FROM programs WHERE school_id = $1', [id]);
            const progIds = programs.rows.map(p => p.id);
            if (progIds.length > 0) {
                // Majors and specializations under these programs
                await client.query('DELETE FROM majors WHERE program_id = ANY($1)', [progIds]);
                await client.query('DELETE FROM specializations WHERE program_id = ANY($1)', [progIds]);
            }
        }
...
exports.deleteProgram = async (req, res) => {
    ...
    try {
        ...
        // Check for students
        const studentCheck = await client.query(
            'SELECT count(*) FROM student_basic_details WHERE program_id = $1',
            [id]
        );
        const studentCount = parseInt(studentCheck.rows[0].count, 10);
        ...
        if (studentCount > 0 && !cascade) {
            await client.query('ROLLBACK');
            return sendConflict(res, 'Cannot delete program: students are associated. Use cascade=true to delete them all.');
        }

        if (studentCount > 0 && cascade) {
            await deleteStudentsByCriteria(client, 'program_id = $1', [id]);
        }

        // Delete associated majors and specializations
        await client.query('DELETE FROM majors WHERE program_id = $1', [id]);
...
exports.deleteMajor = async (req, res) => {
    ...
    try {
        ...
        // Check for students
        const studentCheck = await client.query(
            'SELECT count(*) FROM student_basic_details WHERE major_id = $1',
            [id]
        );
        const studentCount = parseInt(studentCheck.rows[0].count, 10);
        ...
        if (studentCount > 0 && !cascade) {
            await client.query('ROLLBACK');
            return sendConflict(res, 'Cannot delete major: students are associated. Use cascade=true to disconnect them.');
        }

        if (studentCount > 0 && cascade) {
            // "Soft" disconnect: set major_id to null for these students
            await client.query(
                'UPDATE student_basic_details SET major_id = NULL WHERE major_id = $1',
                [id]
            );
        }
...
exports.deleteMinor = async (req, res) => {
    ...
    try {
        ...
        // Check for students
        const studentCheck = await client.query(
            'SELECT count(*) FROM student_basic_details WHERE minor_id = $1',
            [id]
        );
        const studentCount = parseInt(studentCheck.rows[0].count, 10);
        ...
        if (studentCount > 0 && !cascade) {
            await client.query('ROLLBACK');
            return sendConflict(res, 'Cannot delete minor: students are associated. Use cascade=true to disconnect them.');
        }

        if (studentCount > 0 && cascade) {
            // "Soft" disconnect: set minor_id to null for these students
            await client.query(
                'UPDATE student_basic_details SET minor_id = NULL WHERE minor_id = $1',
                [id]
            );
        }
...
exports.deleteSpecialization = async (req, res) => {
    ...
    try {
        ...
        // Check for students
        const studentCheck = await client.query(
            'SELECT count(*) FROM student_basic_details WHERE specialization_id = $1',
            [id]
        );
        const studentCount = parseInt(studentCheck.rows[0].count, 10);
        ...
        if (studentCount > 0 && !cascade) {
            await client.query('ROLLBACK');
            return sendConflict(res, 'Cannot delete specialization: students are associated. Use cascade=true to disconnect them.');
        }

        if (studentCount > 0 && cascade) {
            // "Soft" disconnect: set specialization_id to null for these students
            await client.query(
                'UPDATE student_basic_details SET specialization_id = NULL WHERE specialization_id = $1',
                [id]
            );
        }
